...
//go:generate go run github.com/cilium/ebpf/cmd/bpf2go -cc clang TcDump ./bpf/tc_dump.c -- -I../header
...
type Collect struct {
	Prog    *ebpf.Program `ebpf:"tc_dump"`
...
func main() {
	...
	// Add Qdisc to the link
	if err := netlink.QdiscAdd(qdisc); err != nil {
		fmt.Printf("Failed to Add Qdisc to the link: %v\n", err)
		os.Exit(1)
	}
	...
	defer func() {
		// Detach the TC program from the interface
